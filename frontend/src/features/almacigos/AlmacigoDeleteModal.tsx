...
export default function AlmacigoDeleteModal({
  open,
  loading,
  description,
  onCancel,
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-end justify-center bg-black/40 backdrop-blur-[2px] sm:items-center sm:p-4">
      <div className="flex max-h-[90dvh] w-full flex-col rounded-t-[28px] bg-white shadow-2xl sm:max-h-[90vh] sm:max-w-md sm:rounded-[28px]">
        <div className="flex shrink-0 items-start justify-between border-b border-black/5 px-5 py-5">
          <div>
          ...
          <button
            ...
            onClick={onCancel}
            disabled={loading}
            className="flex h-10 w-10 items-center justify-center rounded-xl text-[#777F7A] transition hover:bg-[#F3F5F3] disabled:opacity-50"
          >
            <X size={20} />
          </button>
        </div>
        ...
        <div className="shrink-0 border-t border-black/5 bg-white px-5 pt-3 pb-[calc(env(safe-area-inset-bottom)+16px)] sm:pb-5">
          <div className="flex gap-3">
            ...
            <button
              ...
              onClick={onConfirm}
              disabled={loading}
              className="h-12 flex-1 rounded-2xl bg-red-600 text-sm font-semibold text-white shadow-[0_8px_24px_rgba(220,38,38,0.18)] transition hover:bg-red-700 disabled:opacity-50"
            >
              {loading
                ? "Eliminando..."
                : "Eliminar"}
